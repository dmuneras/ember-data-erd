...
class NativeJsClassAnalyzer {
  ...
  _readModelDataFromCodeLines(
    ...
    lines.forEach((line, index) => {
      ...
      if (
        NATIVE_CLASS_SYNTAX_KEYWORD_PATTERN.test(line) &&
        checkExtendedClass
      ) {
    ...
    if (checkExtendedClass && baseModelName) {
      ...
      if (!EMBER_DATA_AND_FRAGMENTS_BASE_CLASSES.includes(baseModelName)) {
        ...
        if (parentClassDefinitionPath.includes('models/')) {
          parentClassDefinitionPath = parse(parentClassDefinitionPath).name;
        }

        const baseModelabsolutePath = join(
          modelDefinitionsDirectory,
          `${parentClassDefinitionPath}.js`
        );

        if (existsSync(baseModelabsolutePath)) {
          const baseModelSourceCode = readFileSync(
...
export default NativeJsClassAnalyzer;
